...
const TEMPLATES = {
  석공: {
    s2_1: "설계도 전달 및 줄눈 간격 검토",
    s2_2: "앵글 브라켓 위치 먹매김 후 석재 가조립",
    s2_3: "앵글 고정 → 석재 붙임 → 에폭시 충전 → 최종 고정 상호확인",
    s3_1: "석재 인양 시 와이어로프 체결 상태 확인",
    s3_2: "고소작업 안전대 착용 및 낙하물 방지망 점검",
    s3_3: "줄눈 간격 불일치 시 즉시 작업중지 후 상호 확인",
  },
  비계: {
    s2_1: "시스템 비계 설치 도면 전달",
    s2_2: "벽체 앙카 위치 먹매김 및 작업 계획 전달",
    s2_3: "수직재→수평재→가새 순서 시공순서도 교육 상호확인",
    s3_1: "비계 설치도 확인 후 작업",
    s3_2: "비계공사 안전대 걸이 중요성 설명 (월타이 간격 준수)",
    s3_3: "작업발판 틈새·강풍 시 작업중지 상호 확인",
  },
  코킹: {
    s2_1: "줄눈 청소 및 백업재 삽입 설계도 전달",
    s2_2: "마스킹 테이프 부착 후 코킹재 충전 작업계획 전달",
    s2_3: "헤라 마감 → 테이프 제거 시공순서도 교육 상호확인",
    s3_1: "코킹재 유효기간 및 기온 5도 이하 작업 금지",
    s3_2: "밀폐공간 환기 철저 시공방법 설명",
    s3_3: "마감면 오염 방지 양생 확인 상호 확인",
  },
  트러스: {
    s2_1: "트러스 부재 규격 도면 대조 전달",
    s2_2: "앙카 볼트 고정 위치 작업계획 전달",
    s2_3: "트러스 거치 → 수평 확인 → 볼트 체결 시공순서도 상호확인",
    s3_1: "트러스 인양 시 2점 이상 결속 확인",
    s3_2: "고소작업 안전대 부착설비 사전 확인 설명",
    s3_3: "볼트 미체결 상태 발판 사용 금지 상호 확인",
  },
  양중: {
    s2_1: "양중 장비 일일 점검 도면 전달",
    s2_2: "인양 하중·줄걸이 방법 작업계획 전달",
    s2_3: "신호수 배치 및 신호 방법 시공순서도 상호확인",
    s3_1: "와이어로프 마모·꼬임 상태 점검",
    s3_2: "정격하중 초과 인양 절대 금지 설명",
    s3_3: "인양물 아래 출입통제 상호 확인",
  },
  철거: {
    s2_1: "철거 대상 범위 도면 전달",
    s2_2: "가설 지지대 설치 후 작업계획 전달",
    s2_3: "마감재→설비→구조체 순 철거 시공순서도 상호확인",
    s3_1: "분진 발생 시 방진마스크 착용",
    s3_2: "전기·가스 차단 여부 사전 확인 설명",
    s3_3: "낙하물 방지 및 붕괴 위험 상시 확인 상호 확인",
  },
};

const defaultRows = () => PRESET.map(name => ({ id: UID(), name, workers: "", work: "", note: "" }));
const defaultState = () => ({ date: TODAY(), site: lastSite(), manager: lastManager(), weather: "", mainWork: "", special: "", rows: defaultRows() });
...
export default function App() {
  ...
  useEffect(() => { save(state); if (state.site) localStorage.setItem(SITE_KEY, state.site); if (state.manager) localStorage.setItem(MANAGER_KEY, state.manager); }, [state]);
  ...
  const generateTBM = () => {
    ...
    const subject = names.join(", ") + " 작업";
    const workerDetail = active.map(r => `${r.name} ${r.workers}명`).join(", ");
    setTbm({
      subject,
  ...
  const handleNewDay = () => { if (window.confirm("새 날짜로 초기화할까요?")) { setState({ ...defaultState(), date: TODAY(), site: lastSite(), manager: lastManager() }); setShowOutput(false); setShowTBM(false); setPhotos([]); } };
  ...
  const c = {
    ...
    tab: a => ({ flex:1, padding:"12px 0", fontSize:13, fontWeight:a?600:400, color:a?"#1a73e8":"#666", background:"none", border:"none", borderBottom:a?"2px solid #1a73e8":"2px solid transparent", cursor:"pointer" }),
    ...
    btn: (bg, cl) => ({ width:"100%", padding:13, fontSize:15, fontWeight:600, background:bg, color:cl, border:bg==="none"?`1px solid ${cl}`:"none", borderRadius:10, marginBottom:8, cursor:"pointer" }),
    ...
    tinput: { width:"100%", padding:"8px 10px", fontSize:13, border:"1px solid #ddd", borderRadius:6, marginBottom:8, fontFamily:"inherit", boxSizing:"border-box" },
    tarea: { width:"100%", padding:"8px 10px", fontSize:13, border:"1px solid #ddd", borderRadius:6, marginBottom:8, fontFamily:"inherit", resize:"vertical", lineHeight:1.6, boxSizing:"border-box" },
  };

  // 인쇄용 스타일
  const ps = {
    page: { fontFamily:"'Malgun Gothic','맑은 고딕',sans-serif", fontSize:11, padding:"16px 20px", background:"#fff" },
    top: { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:4 },
    title: { textAlign:"center", fontSize:17, fontWeight:700, letterSpacing:3, padding:"8px 0 10px" },
    signWrap: { border:"1px solid #000", display:"grid", gridTemplateColumns:"24px repeat(3,60px)", gridTemplateRows:"26px 48px" },
    signCell: { border:"1px solid #000", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:600, textAlign:"center", lineHeight:1.3 },
    t: { width:"100%", borderCollapse:"collapse" },
    td: { border:"1px solid #000", padding:"4px 7px", fontSize:11 },
    th: { border:"1px solid #000", padding:"4px 7px", fontSize:11, background:"#f0f0f0", fontWeight:700, textAlign:"center" },
    eduWrap: { border:"1px solid #000", borderTop:"none", display:"flex" },
    eduSide: { writingMode:"vertical-rl", textAlign:"center", fontWeight:700, fontSize:11, background:"#f0f0f0", padding:"6px 3px", borderRight:"1px solid #000", letterSpacing:4, minWidth:20 },
    eduInner: { flex:1 },
    eduHdr: { display:"flex", borderBottom:"1px solid #000", background:"#f0f0f0" },
    eduH1: { width:150, fontWeight:700, textAlign:"center", padding:"4px 6px", borderRight:"1px solid #000", fontSize:11, flexShrink:0 },
    eduH2: { flex:1, fontWeight:700, textAlign:"center", padding:"4px 6px", fontSize:11 },
    eduRow: { display:"flex", borderBottom:"1px solid #000" },
    eduRowLast: { display:"flex" },
    eduLbl: { width:150, borderRight:"1px solid #000", padding:"6px", fontSize:11, lineHeight:1.6, flexShrink:0, background:"#fafafa" },
    eduSubLbl: { fontSize:11, fontWeight:600, color:"#333", marginBottom:2 },
    eduCon: { flex:1, padding:"6px 8px", fontSize:11, lineHeight:1.8 },
    subItem: { marginBottom:6 },
    photoWrap: { border:"1px solid #000", borderTop:"none", display:"flex" },
    photoSide: { writingMode:"vertical-rl", textAlign:"center", fontWeight:700, fontSize:11, background:"#f0f0f0", padding:"6px 3px", borderRight:"1px solid #000", letterSpacing:4, minWidth:20 },
    photoGrid: { flex:1, display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, padding:8, minHeight:150 },
    photoImg: { width:"100%", height:130, objectFit:"cover", border:"1px solid #ccc" },
    photoEmpty: { width:"100%", height:130, background:"#f5f5f5", border:"1px dashed #ccc" },
  };
  ...
  return (
    <>
      <style>{`
        ...
        input, textarea { font-family:inherit; font-size:15px; width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; background:#fff; color:#1a1a1a; outline:none; box-sizing:border-box; }
      ...
      <div style={c.wrap} className="no-print">
        ...
        <div style={c.tabs}>
          ...
          <button style={c.tab(tab==="settings")} onClick={() => setTab("settings")}>⚙️ 설정</button>
        </div>
        ...
        <div style={c.body}>
          {tab === "write" && <>
            ...
            <div style={c.card}>
              ...
              <select value={state.site} onChange={e => set("site", e.target.value)} style={{ width:"100%", padding:"10px 12px", fontSize:15, border:"1px solid #ddd", borderRadius:8, background:"#fff", marginBottom:10 }}>
            ...
            {showOutput && (
              <div style={c.card}>
                ...
                <button style={{ ...c.btn("#f0f4ff","#1a73e8"), marginTop:8, marginBottom:0 }} onClick={handleCopy}>{copied?"✓ 복사됨!":"📋 클립보드 복사"}</button>
              </div>
            )}
            ...
            {showTBM && (
              <div style={{ ...c.card, border:"1px solid #34a853", background:"#f0fff4", marginTop:4 }}>
                ...
                <input style={c.tinput} value={tbm.subject} onChange={e => setT("subject", e.target.value)} />

                <div style={{ fontWeight:600, fontSize:13, color:"#333", margin:"8px 0 6px" }}>1. 당일작업의 공법이해</div>
                <label style={c.lbl}>- 작업개요</label>
                <input style={c.tinput} value={tbm.s1_1} onChange={e => setT("s1_1", e.target.value)} />
                <label style={c.lbl}>- 작업내용</label>
                <input style={c.tinput} value={tbm.s1_2} onChange={e => setT("s1_2", e.target.value)} />
                <label style={c.lbl}>- 인원투입 등</label>
                <input style={c.tinput} value={tbm.s1_3} onChange={e => setT("s1_3", e.target.value)} />

                <div style={{ fontWeight:600, fontSize:13, color:"#333", margin:"8px 0 6px" }}>2. 시공 상세도면에 따른 세부 시공순서</div>
                <label style={c.lbl}>- 설계도면</label>
                <textarea style={c.tarea} rows={2} value={tbm.s2_1} onChange={e => setT("s2_1", e.target.value)} />
                <label style={c.lbl}>- 작업계획</label>
                <textarea style={c.tarea} rows={2} value={tbm.s2_2} onChange={e => setT("s2_2", e.target.value)} />
                <label style={c.lbl}>- 시공순서도 등에 의한 교육</label>
                <textarea style={c.tarea} rows={2} value={tbm.s2_3} onChange={e => setT("s2_3", e.target.value)} />

                <div style={{ fontWeight:600, fontSize:13, color:"#333", margin:"8px 0 6px" }}>3. 시공기술상의 주의사항</div>
                <label style={c.lbl}>- 특이사항(공법 등)</label>
                <textarea style={c.tarea} rows={2} value={tbm.s3_1} onChange={e => setT("s3_1", e.target.value)} />
                <label style={c.lbl}>- 작업지휘자의 시공방법 설명</label>
                <textarea style={c.tarea} rows={2} value={tbm.s3_2} onChange={e => setT("s3_2", e.target.value)} />
                <label style={c.lbl}>- 시공순서도 등에 의한 상호 협의</label>
                <textarea style={c.tarea} rows={2} value={tbm.s3_3} onChange={e => setT("s3_3", e.target.value)} />

                <label style={c.lbl}>현장 사진</label>
                <input type="file" accept="image/*" multiple ref={photoRef} style={{ display:"none" }} onChange={handlePhoto} />
                <button style={{ ...c.btn("#fff","#34a853"), border:"1px solid #34a853", marginBottom:8 }} onClick={() => photoRef.current.click()}>📷 사진 추가</button>
                {photos.length > 0 && (
          ...
          {tab === "history" && <>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
              {["전체", ...sites].map(s => (
                <button key={s} onClick={() => setHistFilter(s)}
                  style={{ fontSize:12, padding:"5px 10px", borderRadius:16, border:"1px solid", borderColor: histFilter===s?"#1a73e8":"#ddd", background: histFilter===s?"#e8f0fe":"#fff", color: histFilter===s?"#1a73e8":"#666", cursor:"pointer" }}>
                  {s}
                </button>
              ))}
            </div>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"4mm" }}>
          ...
          <table style={{ borderCollapse:"collapse" }}>
            <tbody>
              ...
              <tr>
                <td style={{ border:"1px solid #000", height:"14mm", width:"18mm" }}></td>
                <td style={{ border:"1px solid #000", height:"14mm", width:"18mm" }}></td>
                <td style={{ border:"1px solid #000", height:"14mm", width:"18mm" }}></td>
              </tr>
            </tbody>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ display:"flex", borderBottom:"1px solid #000" }}>
              ...
              <div style={{ flex:1, padding:"3mm 4mm", lineHeight:1.9 }}>
                <div>- 작업개요</div>
                <div style={{ paddingLeft:"4mm", marginBottom:"2mm" }}>{tbm.s1_1}</div>
                <div>- 작업내용</div>
                <div style={{ paddingLeft:"4mm", marginBottom:"2mm" }}>{tbm.s1_2}</div>
                <div>- 인원투입 등</div>
                <div style={{ paddingLeft:"4mm" }}>{tbm.s1_3}</div>
              </div>
            </div>
            {/* 2번 */}
            <div style={{ display:"flex", borderBottom:"1px solid #000" }}>
              ...
              <div style={{ flex:1, padding:"3mm 4mm", lineHeight:1.9 }}>
                <div>- 설계도면</div>
                <div style={{ paddingLeft:"4mm", marginBottom:"2mm" }}>{tbm.s2_1}</div>
                <div>- 작업계획</div>
                <div style={{ paddingLeft:"4mm", marginBottom:"2mm" }}>{tbm.s2_2}</div>
                <div>- 시공순서도 등에 의한 교육</div>
                <div style={{ paddingLeft:"4mm" }}>{tbm.s2_3}</div>
              </div>
            </div>
            {/* 3번 */}
            <div style={{ display:"flex" }}>
              ...
              <div style={{ flex:1, padding:"3mm 4mm", lineHeight:1.9 }}>
                <div>- 특이사항(공법 등)</div>
                <div style={{ paddingLeft:"4mm", marginBottom:"2mm" }}>{tbm.s3_1}</div>
                <div>- 작업지휘자의 시공방법 설명</div>
                <div style={{ paddingLeft:"4mm", marginBottom:"2mm" }}>{tbm.s3_2}</div>
                <div>- 시공순서도 등에 의한 상호 협의</div>
                <div style={{ paddingLeft:"4mm" }}>{tbm.s3_3}</div>
              </div>
            </div>
          </div>
        </div>
